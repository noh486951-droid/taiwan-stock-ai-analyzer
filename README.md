...
| 自選股 | `watchlist.html` | 個股管理、支撐壓力位/停損建議、籌碼集中度、AI 分析彈窗 |
...
### v9.3 (2026-04-13)
**雙 AI 備援體制：Mistral AI 整合與穩定性優化**
- **Mistral AI 備援機制**：
  - 成功整合 Mistral AI 作為 Gemini 配額耗盡時的自動替代方案（僅限個股分析）。
  - 支援多把 Mistral API Key 輪詢，並同步更新 GitHub Actions (Python) 與 Cloudflare Worker (JS) 以支援此備援模式。
- **穩定性優化 (Rate Limiting)**：
  - 在後台分析中加入 2 秒延遲冷卻時間，確保在觸發 Mistral 備援時不會因為請求過於頻繁（RPM）而導致分析中斷。
- **UI 操作體驗升級**：
  - **個股即時重整鈕**：在自選股彈窗中新增「🚀 立即嘗試備援分析 (Mistral)」按鈕，當原本分析失敗或資料過舊時，使用者可手動點擊即時獲取最新 Mistral 診斷結果。
  - **查詢介面優化**：主頁「個股快速查詢」現已無縫銜接雙 AI 引擎，確保全天候 24 小時皆可產出分析報告。
