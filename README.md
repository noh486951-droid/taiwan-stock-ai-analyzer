...
**功能大進化：自選股群組化與模型終極升級**
- **自選股群組管理**：
  - 新增「群組分頁」功能，支援多個獨立的個人自選清單（如：核心持股、觀察名單、高股息等）。
  - 提供完整群組管理介面：可自由新增、重新命名、刪除群組。
  - 自動遷移舊版單一清單至預設群組，確保資料不遺失。
- **Gemini-3-Flash 全面導入**：
  - Worker 即時分析引擎預設切換至最新的 `gemini-3-flash` 模型，提供更快的反應速度與更精準的盤勢判讀。
- **UI 穩定性修復**：
  - 修復了自選股详细診斷彈窗在特定情況下無法開啟的程式碼衝突 Bug。
  - 優化群組切換時的資料載入流暢度。
