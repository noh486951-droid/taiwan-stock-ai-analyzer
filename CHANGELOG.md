...
|
### v11.13.8 (2026-05-12)
**RS 進場過濾 + 版本更新自動通知 + AI 對話短期記憶**
- **虛擬投資引擎優化**：
  - **RS 進場過濾 (RS Entry Filter)**：新增 Relative Strength 雙重檢查。自動拒絕「弱勢/極弱」或「落後大盤 > 1.5%」的進場訊號，優先佈局強勢領漲股。原因碼：`rs_weak_entry`。
- **CI/CD 穩定性與監控**：
  - **版本更新自動通知**：GitHub Actions 實裝版本偵測腳本。任何非自動更新的 Release Commit 會自動推送紫色通知卡片至 Discord `#🔧-系統健康` 頻道。
- **Discord Bot 智慧升級**：
  - **對話連貫性 (Conversation Memory)**：導入 KV 短期記憶機制（最多 4 輪，TTL 30 分鐘）。AI 現在能記住先前的問題，支援「承上啟下」式諮詢。
  - **記憶清除指令**：支援 `/ask reset` 或關鍵字（清除、新對話）重置上下文。
### v11.13.4 (2026-05-11)
...
- **收盤總結數據升級**：
  - **今日總計 (Daily Total)**：於盤後總結與 Discord 報表中新增「今日總損益」欄位，方便掌握帳戶表現。
- **數據透視強化**：
  - **營收 YoY 動態顯示**：在自選股診斷中更直觀地整合「營收年增率」數據。
  - **大戶持股 Top 追蹤**：實裝「大戶持股比例變化」追蹤機制，自動列出持倉中大戶最顯著集中的個股。

### v11.12 (2026-05-09)
**AI 早安主播 + Discord 互動按鈕 + PNG 視覺化 + 月報系統**
- **AI 主播風格快報 (Phase A)**：
  - **晨間 AI 主播**：每日開盤前自動生成主播風格的 `morning_digest.json`。
  - **專屬頻道推送**：自動推送到 Discord `#🌍-總經情報` 頻道，提供更有溫度的晨間觀點。
- **Discord Bot 指令大擴充 (Phase B/C)**：
  - **6 個新指令**：新增 `/history` (最近 10 筆平倉)、`/winners` (歷來贏家)、`/losers` (歷來輸家)、`/risk` (風險穿透)、`/streak` (連勝/連敗統計)、`/refresh` (手動觸發 workflow)。
  - **DM 友善別名**：新增 `/chat` 作為 `/ask` 的別名，方便在私訊中使用。
- **互動按鈕實裝 (Phase D)**：
  - **卡片連動**：Discord 的進場、出場、階梯預警、鴨子警示卡片現在都附帶「📊 查現況」、「💬 問 AI」、「📜 查歷史」等即時按鈕，點擊即可觸發 Bot 回覆。
- **視覺化數據推送 (Phase E)**：
  - **PNG 圖表生成**：隨每日總結推送「總資產曲線」、「月勝率統計」與「持倉圓餅圖」之視覺化圖表（需搭配後端繪圖模組）。
- **月報與評分系統 (Phase F)**：
  - **自動月報**：每月最後一個交易日自動產生月度總結，由 AI 給出 1-10 分的綜合評分與深度檢討。
...
- **無縫退回機制**：若遇到對照表未收錄之個股，將自動退回使用原英文名稱，避免報錯。

### v11.10.6 (2026-05-08)
**Discord 卡片視覺體驗升級 (ANSI 著色)**
- **台股專屬紅綠配色**：利用 Discord 內建的 ANSI escape codes，將「出場報告」、「每日總結」及「週報」中的損益金額全面加上極度醒目的顏色標籤（紅色字體 = 獲利/上漲，綠色字體 = 虧損/下跌）。
- **雙重視覺指示**：除了文字著色外，標題及內文全面配上 🔴/🟢/⚪ emoji，搭配原本的卡片左側邊框顏色（紅=賺、綠=賠），打造台股直覺式的極致視覺化財報體驗。
...
- **DC 卡片長文多頁推送**：
  - **解鎖字數上限**：前端將完整分析長文（≤14000 字）送至 Worker，不再被單一 Embed 截斷。
  - **自動分片推播**：Worker 將長文動態切分（每 3500 字為一片段），並利用 Discord webhook 機制推播最多 4 條完整連續訊息，確保分析內容 100% 不漏接。
- **強制量化結構模板 (Force Numeric Prompt)**：
  - **結構化五大關鍵價位**：全面改寫 Prompt，強制 AI 使用統一 Markdown 結構產出「加碼點、減碼點、停利、停損、明日盯盤點」，拒絕流水帳。
  - **禁止模糊語句**：透過身份設定（Role Framing）嚴禁 AI 說出「視情況」、「再觀察」等無承擔詞彙，要求「數字優先於形容詞」。
  - **整體帳戶警戒線與自評**：新增「整體警戒線」要求（跌到特定金額強制全面減倉），並附帶 AI 針對本次選股品質的 1-10 分自評。
...
- **首字 Timeout 寬容化 (Warmup Buffer)**：
  - **兩段式計時**：針對 Gemini 大 Prompt 熱身慢的特性，首字超時放寬至 60 秒（諮詢）/ 50 秒（一般），收到首字後緊縮回 25 秒，解決「0 字重試」死循環。
  - **狀態透視**：將原有的「思考中」細分為「思考中…（可能要 30 秒以上）」與「⏳ AI 正在生成…」，消除使用者的連線焦慮。
- **對話流暢度與穩定性**：
  - **諮詢 One-shot 化**：AI 持倉諮詢改為「單次請求」模式，不攜帶聊天歷史。避免 Prompt 隨重試不斷累積膨脹，越噴越慢。
  - **諮詢 Payload 極致精簡**：持倉諮詢 Payload 精簡 60%。RS 指標與族群流向壓製為單一字串，歷史紀錄由 20 筆減至 8 筆，徹底解決大 Prompt 造成的逾時。
  - **錯誤訊息精準化**：區分「AI 還沒開始回覆」與「生成中斷」，並在失敗時顯示當前已收到的部分內容。
- **後端穩定性強化 (Worker Stability)**：
  - **超時放寬**：Worker 端上游 Fetch 超時由 15s 放寬至 25s，容許 Gemini 在安全審核階段的長延遲。
  - **模型換代**：切換至 gemini-2.5-flash 穩定版，減少 503 報錯機率。
- **聊天歷史管理 (History Cleanup)**：
  - **🧹 清除歷史按鈕**：UI 新增「清除聊天歷史」按鈕，手動重置 chatHistory。
  - **自動修復**：連線失敗時自動 pop() 掉最後一個 User Prompt，防止重試時 Prompt 疊加。
  - **防重疊機制**：重試時自動移除舊的錯誤訊息按鈕。
...
- **11 大智慧推送觸發點**：
  - **進出場即時通知**：📥 進場 (藍)、📤 出場/賺 (紅)、📤 出場/賠 (綠)。每筆成交立即推送。
  - **風控與預警**：
    - **⚠️ 階梯預警**：每 5 點（+3/+5/+7/+10/+15 或 -2/-4/-6/-8/-10）觸發一次 (黃/藍)。
    - **🦆 鴨子要飛了**：回吐預警。當獲利峰值 ≥7% 且回吐 ≥5pp 時發出深紅預警（每倉一次）。
    - **🌅 尾盤摜壓**：14:00 偵測尾盤異常且持有該股時發出紅字警告。
  - **盤勢與量能**：
    - **⚡ 量能激增**：ratio ≥2 且 Bullish 時發出黃色通知（每檔每日一次）。
    - **🌍 隔日總經提醒**：17-19 點推送隔日高/中重要度事件 (黃)。
  - **自動 Review 與總結**：
    - **🔧 AI Review**：18:00 自動調整停損/目標價通知 (橘)。
    - **📊 每日盤後總結**：18:00 定時推送每日績效與盤勢 (紫)。
    - **📅 週五週報**：週五 18:00 推送本週績效回顧 (深紫)。
- **配色與互動**：全面對齊台股紅漲綠跌色系，採用 Discord Embed Card 結構化展示。

---

### v11.9 (2026-05-06)
**全方位交易支援：總經行事曆 + 回測模擬器 + ETF 穿透 + 產業熱力圖**
- **全球總經行事曆 (#3)**：整合 FOMC 利率會議、NFP 非農、CPI 物價指數與大廠（如 NVDA）財報。AI 分析會自動引用未來 7 天關鍵事件，在大事件前自動調整信心度評分（防呆）。
- **策略回測模擬器 (#5)**：`paper_trade.html` 新增「🧪 策略回測模擬器」。支援純前端即時運算，可針對 6 維參數（信心門檻、停損、移動停利、MA5 乖離等）進行歷史績效即時對比。
- **ETF 穿透 + 風險集中度 (#1)**：支援 10 大熱門 ETF（如 0050, 0056, 00878 等）成分股透視。當持倉 ETF 與個股在同產業總曝險 > 50% 時，觸發紅字「過度曝險警告」。
- **產業輪動 5-10 日熱力圖 (#2)**：`sectors.html` 新增「🌀 10 日資金輪動熱力圖」。透過 `track_sector_history.py` 每日紀錄類股強弱，視覺化呈現資金輪動路徑。
- **盤中量能激增 + Fast Track (#6)**：當偵測到量能比 ≥ 2.0× 時，系統標註「⚡ 盤中量能激增」，自動化引擎會跳過第二次訊號確認（Fast Track），優先搶進強勢爆量股。
- **尾盤 5 分鐘量價警示 (#7)**：`track_closing_action.py` 於 14:00 自動執行。分析 13:25-13:30 尾盤搓合段的異常拉抬或摜壓，預警隔日開盤風險。
- **部署與數據**：
  - 新增 `data/macro_calendar.json`、`data/etf_holdings.json`、`data/sector_history.json`。
  - 所有新功能均具備 `continue-on-error` 機制，確保 Workflow 穩健運行。
