...
|
### v11.13 (2026-05-11)
**收盤總結強化 + 營收 YoY 整合 + 大戶持股 Top 追蹤**
- **收盤總結數據升級**：
  - **今日總計 (Daily Total)**：於盤後總結與 Discord 報表中新增「今日總損益」欄位，方便一眼掌握單日帳戶表現。
- **數據透視強化**：
  - **營收 YoY 動態顯示**：在自選股診斷中更直觀地整合「營收年增率」數據，AI 會自動對比歷史動能與最新表現。
  - **大戶持股 Top 追蹤**：實裝「大戶持股比例變化」追蹤機制，自動列出持倉中大戶最顯著集中的個股，強化籌碼面選股依據。
- **後端引擎優化**：
  - **Scout 雷達精準化**：優化 `scout.py` 數據過濾邏輯，減少雜訊干擾。
  - **交易引擎風控調整**：微調 `paper_trade_engine.py` 之出場信心度權重。
