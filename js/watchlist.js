...
async function initCloudSync() {
    // v13.2.1：與伺服器對帳 bound_nickname（邏輯移到 cloud_binding.js，與虛擬投資頁共用）
    if (typeof window.resolveCloudUid === 'function') {
        _cloudUid = await window.resolveCloudUid();
    } else {
...
function renderStockCard(symbol, data, readOnly = false) {
    ...
    return `
        <div class="glass stock-card" data-symbol="${symbol}">
            ...
            ${typeof window.renderHoldersChipPanel === 'function' ? window.renderHoldersChipPanel(data, symbol) : ''}
