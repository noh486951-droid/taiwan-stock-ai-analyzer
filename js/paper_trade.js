...
【強制輸出格式：操盤手要看數字，不看廢話】
- 任何「決策」都要附**具體價位 / 百分比 / 距離**（例：「$245 出場」「漲到 +5% 賣半倉」）
- 禁止說「視情況」「再觀察」這種沒承擔的詞
- 必須給「**進場後 5 個關鍵價位**」：加碼點、減碼點、停利、停損、明日觀察點
- 如果某檔現在 RS 弱 / 族群弱 / 浮虧 → **明確說「我會在 $X 出場」**，不要逃避
- 數字優先於形容詞：寫「跌破 $X 出場」而不是「跌破關鍵支撐就考慮出場」
`.trim();
...
⚠️ **重要規則：每一檔回覆必須包含具體數字（價位、百分比、距離）**，不能只用文字描述。

對每一檔請用以下「**結構化模板**」回答（每一項都要有明確數字）：

\`\`\`
### N. {代號} ({中文名})
- 📍 **進場 / 現價 / 浮動損益**：$XXX → $XXX（+/-X.X%）
- 🎯 **我的決策**：續抱 / 減碼 X 成 / 立即出場（三選一）
- 💰 **若續抱，明確價位**：
    • 加碼點 $XXX（觸發條件：例如 MA20 站穩、量比 > 1.5）
    • 減碼點 $XXX（觸發條件：例如跌破 MA10 或 RS 轉弱）
    • 停利價 $XXX（漲到此先賣 X 成）
    • 停損價 $XXX（跌破此立即出清，不留情面）
- ⚠️ **若選擇出場，建議成交區間**：$XXX ~ $XXX（尾盤掛單 / 開盤掛單，二擇一）
- 🔔 **本週盯這 1 個關鍵數字**：例如 RSI 是否守 50 / MA5 是否守 $XXX / 量能是否萎縮到 X 萬張以下
- 📝 **理由**：用 1-2 句解釋為何給這個數字（引用 RS 標籤、族群強度、AI 信心）
\`\`\`

最後寫「📊 整體判讀」段落（**也要有數字**）：
- 帳戶整體浮盈/浮虧：$X,XXX (X.XX%)
- 整體警戒線：當帳戶損益跌到 $X,XXX 以下要全面減倉
- 今日市場族群資金集中在：（引用 top 3 sectors 名稱 + 漲跌幅）
- 我的持倉踩對邊比例：X 檔對位 / Y 檔逆勢
- 一句話打分：本次選股品質 X/10 分，系統訊號 [有效 / 部分有效 / 無效]
- 明日操作優先順序：1. 先處理 XXX；2. 觀察 XXX 加碼；3. 等 XXX 訊號`;
